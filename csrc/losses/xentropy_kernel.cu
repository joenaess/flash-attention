...
__global__ void xentropy_cuda_kernel(
    ...
    // Load pred tile into shared memory cooperatively and contiguously
    size_t total_pred_elements = TILE_SIZE * D;
    for (size_t i = threadIdx.x; i < total_pred_elements; i += blockDim.x) {
        size_t r = i / D;
        size_t c = i % D;
        size_t global_m = blockIdx.x * TILE_SIZE + r;
        if (global_m < M) {
            s_pred[r * D + c] = pred[global_m * D + c];
        } else {
            s_pred[r * D + c] = 0.0;
        }
    }
    __syncthreads(); // Wait for pred tile to be fully loaded
...
void launch_xentropy_kernel(
    ...
    size_t shared_mem_size = 2 * TILE_SIZE * D * pred.element_size();

    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, pred.scalar_type(), "xentropy_cuda_kernel", ([&] {
        cudaFuncSetAttribute(xentropy_cuda_kernel<scalar_t>, cudaFuncAttributeMaxDynamicSharedMemorySize, shared_mem_size);
