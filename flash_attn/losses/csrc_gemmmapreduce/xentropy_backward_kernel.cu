...
__global__ void xentropy_cuda_backward_kernel(
    ...
    int64_t* s_tixs = (int64_t*)(shared_mem + TILE_SIZE * D * sizeof(scalar_t));
    __shared__ acc_t s_warp_sums[32];
    ...
    for (size_t n_start = 0; n_start < N; n_start += TILE_SIZE) {
        ...
        for (size_t n_step = 0; n_step < TILE_SIZE; ++n_step) {
            ...
            for (size_t d = 0; d < D; ++d) {
                ...
                if (m < M) {
                    local_grad_trg = dh * (acc_t)pred[m * D + d];
                }
                ...
                // Phase 2 (Shared Staging)
                if (lane_id == 0) {
                    s_warp_sums[warp_id] = val;
                }
                __syncthreads(); // Barrier to ensure all warps checked in
                
                // Phase 3 (Inter-Warp Final Sweep)
                if (warp_id == 0) {
                    acc_t warp_val = (lane_id < num_warps) ? s_warp_sums[lane_id] : 0.0;
                    for (int offset = 16; offset > 0; offset /= 2) {
                        warp_val += __shfl_down_sync(0xffffffff, warp_val, offset);
                    }
                    
                    // Phase 4 (Mathematical Global Write)
                    if (lane_id == 0) {
                        gpuAtomicAdd(&grad_trg[actual_n * D + d], (scalar_t)warp_val);
                    }
                }
                __syncthreads(); // Barrier to ensure s_warp_sums is not overwritten in next iteration
            }
        }
...
void launch_xentropy_backward_kernel(
    ...
    // Updated shared memory size calculation (removed s_grad_trg)
    size_t shared_mem_size = TILE_SIZE * D * pred.element_size() + TILE_SIZE * sizeof(int64_t);
